...
describe WeatherApp do
  ...
  describe 'new' do
    it "takes a parameter and returns a weatherApp object" do      
      @weatherApp = WeatherApp.new('glorp')
      expect(@weatherApp).to be_an_instance_of(WeatherApp)
      expect(@weatherApp.word).to eq('glorp')
      expect(@weatherApp.guesses).to eq('')
      expect(@weatherApp.wrong_guesses).to eq('')
    end
  end
